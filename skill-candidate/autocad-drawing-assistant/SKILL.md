...
- For **“开始预览”**, load the user-selected profile with `cad_load_drawing_profile`, require explicit plan confirmation, force new geometry onto `AI_PREVIEW_*` or `AI_UNCERTAIN`, then run `cad_plan_validate` → `cad_execute_plan` → `cad_verify_execution` without changing the plan payload.
- For **“正式提交”**, require a passed preview verification and a fresh confirmation naming the exact preview handles and formal target layers. Before any write, run `D:\AI\multiCAD-mcp\.venv\Scripts\python.exe D:\AI\multiCAD-mcp\scripts\Backup-CAD-Drawing.py --keep 20`; stop if it reports an unsaved or dirty drawing. Transfer only through a revalidated target-handle plan with `preview_mode=false`. If the enhanced executor cannot express the layer transfer, stop; never fall back to legacy entity modification, copying, deletion, or shell-driven COM edits.
...
- For **“撤回本次”**, identify the latest verified AI execution record and prove that its handles belong to the active drawing and that no later user operation could be affected. Use only a task-scoped guarded undo capability. If ownership of the next undo record cannot be proven, or only generic legacy undo is available, stop and explain the blocker; never risk undoing earlier user work.
